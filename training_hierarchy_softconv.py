...
def to_binary(num, bits):
    return np.array(list(np.binary_repr(num, width=bits))).astype(int)

def from_binary(arr):
    return int("".join(arr.astype(str)), 2)

def generate_widebeam(Nt, indices):
    w = np.zeros(Nt, dtype=complex)
    if len(indices) > 0:
        w[indices] = 1
...
def training_hierarchy_softconv(Codebook, h, SNR, Nt, conv_encoder_conf):
    ...
    for ns in range(N_status):
        for nc in range(N_choices):
            ...
            merged = np.concatenate(
                (to_binary(nc,k), to_binary(ns,N_inner_state_bits))
            )
    ...
    for k_iter in range(1,N_input_blocks+1):
        ...
        delta = int(Nt/(2**(k_iter-1)))

        for i in range(N_status):
            start = path_cache_new[i]*delta
            end = (path_cache_new[i]+1)*delta
        ...
        index11 = np.where(idxf1)[0]
        index12 = np.where(idx1)[0]
        index21 = np.where(idxf2)[0]
        index22 = np.where(idx2)[0]
        ...
        for ns in range(N_status):
            for nc in range(N_choices):
                ...
                if k_iter <= 3:
                    ...
                    amp = np.sqrt(2)*np.sqrt(Nt/2/len(index21))
                ...
                if k_iter <= 3:
                    ...
                    amp = np.sqrt(2)*np.sqrt(Nt/2/len(index11))
