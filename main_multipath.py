...
K = 1              # Number of users
L = 1              # Number of paths (LOS considered first)
_lambda = c / f
d = _lambda / 2
N_iter = 1000
theta_min = -np.pi / 2
...
# SNR LIST
# -----------------------------
SNR_dB_list = np.arange(-5, 11, 3)   # -5 to 10 dB in steps of 3
...
for idx, SNR in enumerate(SNR_list):
    ...
    for i_iter in range(N_iter):
        ...
        theta_DoA = np.random.randint(1, Nt+1, size=(3, K))  # 3 paths per user
        
        for k in range(K):
            H[:, k], h_BS[:, k] = generate_channel_multipath(Nt, theta_DoA[:, k], d, _lambda)
...
plt.title('Communication Performance in Multipath mmWave System')
plt.show()
